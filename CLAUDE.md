...
| Wrist yaw | **Modular, period 90 deg.** 0 and 90 are the same orientation. Commanding 0 from 90 is a no-op the servo correctly ignores, and linear verification calls it a desync — so `tcp_yaw` declares NO `feedback_sensor` until RFC 0001 lands |
